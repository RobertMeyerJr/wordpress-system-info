...
<div id=info_tabs>
		<h2 class="nav-tab-wrapper tabbed_nav">
			...
			<a class="nav-tab" href="#tab_images">Image Sizes</a>
			<a class="nav-tab" href="#tab_phpinfo">PHP Info</a>
			<a class="nav-tab" href="#tab_php">PHP</a>	
			<?php  if(function_exists('fpm_get_status') ): ?>
				<a class="nav-tab" href="#tab_phpfpm">PHP FPM</a>
			<?php endif; ?>		
		</h2>	
		...
		<div class=tab id=tab_images>
			...
			?>
			<h3><?=count($all_sizes)?> Image Thumbnail Sizes</h3>
			<table class='widefat striped'>
				<thead>
					<tr>
						<th>Name</th>
				...
				<tbody>						
						<?php foreach($all_sizes as $name=>$i) : ?>		
							<?php 
							$h = in_array($i['height'],['9999']) ? 'auto' : $i['height'];						
							?>
							<tr>
								<td><span style="font-size:2.5em"><?php echo $name?></span></td>
								<td><?php echo ($i['crop'])?'Yes':'No'?></td>
								<td style="text-center">
									<span class="thumb_example" style="width:<?php echo $i['width']?>px;height:<?php echo $h?>px">
									<?php echo $i['width']?>
									x
									<?php echo $i['height']?>
									</span>
...
jQuery(function($){
	...
	$('#info_tabs .nav-tab').click(function(){
		var tab = $(this).attr('href');
		$('#info_tabs .tab').removeClass('active');
		$(tab).addClass('active');
		return false;
	});
})	
